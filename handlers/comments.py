...
def handle_comment_event(payload: dict) -> dict:
    ...
    try:
        ...
        try:
            activity = db.session.query(IssueActivity).filter_by(
                repo_full_name=repo_full_name,
                issue_number=issue_number
            ).first()
            
            if activity:
                activity.last_activity = datetime.now(datetime.timezone.utc)
    ...
    try:
        if cmd == "/close":
            ...
            if size in ["s", "m", "l", "xl"]:
                ...
                github_client.add_labels_to_issue(repo_full_name, issue_number, [size_label])
                return {"status": "success", "action": "size_label_added", "size": size}
            else:
                return {"status": "error", "message": "Invalid size. Use s, m, l, or xl"}
        
        elif cmd == "/priority" and len(parts) > 1:
            priority = parts[1].lower()
            if priority in ["low", "medium", "high", "critical"]:
                priority_label = f"priority:{priority}"
                github_client.add_labels_to_issue(repo_full_name, issue_number, [priority_label])
                return {"status": "success", "action": "priority_label_added", "priority": priority}
            else:
                return {"status": "error", "message": "Invalid priority. Use low, medium, high, or critical"}
        
        elif cmd == "/assign" and len(parts) > 1:
            assignee = parts[1].replace("@", "")  # Remove @ if present
            github_client.assign_users_to_issue(repo_full_name, issue_number, [assignee])
            return {"status": "success", "action": "user_assigned", "assignee": assignee}
