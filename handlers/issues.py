...
async def handle_issue_opened(payload: dict) -> dict:
    ...
    try:
        ...
        if matched_labels:
            github_client.add_labels_to_issue(repo_full_name, issue_number, matched_labels)
        
        # Match owners based on path hints in the body
        matched_owners = rules_manager.match_owners(issue_body)
        if matched_owners:
            github_client.assign_users_to_issue(repo_full_name, issue_number, matched_owners)
...
This information will help us investigate and resolve the issue more quickly."""
            ...
            github_client.add_comment_to_issue(repo_full_name, issue_number, checklist_comment)
