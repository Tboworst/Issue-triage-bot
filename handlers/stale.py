...
def check_stale_issues():
    ...
    with app.app_context():
        try:
            ...
            cutoff_date = datetime.now(datetime.timezone.utc) - timedelta(days=stale_days)
            ...
            stale_issues = db.session.query(IssueActivity).filter(
            ...
            for issue_activity in stale_issues:
                try:
                    ...
                    if issue.state == "open":
                        # Add stale label
                        github_client.add_labels_to_issue(repo_full_name, issue_number, ["stale"])
...
Thank you for your contributions!"""
                        ...
                        github_client.add_comment_to_issue(repo_full_name, issue_number, stale_comment)
                        
                        # Mark as stale in our database
                        issue_activity.is_stale = True
                        db.session.commit()
                        
                        logger.info(f"Marked issue #{issue_number} in {repo_full_name} as stale")
                    
                    else:
                        # Issue is closed, remove from tracking
                        db.session.delete(issue_activity)
                        db.session.commit()
                        logger.info(f"Removed closed issue #{issue_number} from tracking")
                        
                except Exception as e:
                    logger.error(f"Failed to process stale issue {issue_activity.repo_full_name}#{issue_activity.issue_number}: {e}")
                    db.session.rollback()
                    continue
        
            # Clean up very old stale issues (close them after additional time)
            close_days = int(os.getenv("STALE_CLOSE_DAYS", "7"))  # Additional days before closing
            close_cutoff = datetime.utcnow() - timedelta(days=stale_days + close_days)
            
            very_stale_issues = db.session.query(IssueActivity).filter(
                ...
                try:
                    ...
                    if issue.state == "open":
                        ...
                        if "stale" in stale_labels and "pinned" not in stale_labels:
                            ...
                            github_client.add_comment_to_issue(repo_full_name, issue_number, close_comment)
                            github_client.close_issue(repo_full_name, issue_number)
                            
                            logger.info(f"Automatically closed stale issue #{issue_number} in {repo_full_name}")
                    ...
                    logger.error(f"Failed to close stale issue {issue_activity.repo_full_name}#{issue_activity.issue_number}: {e}")
                    db.session.rollback()
                    continue
